package lk.krishan_mobile.asset.payment.controller;
...
import lk.krishan_mobile.asset.good_received_note.service.GoodReceivedNoteService;
...
import lk.krishan_mobile.asset.purchase_order.service.PurchaseOrderService;
  ...
  public String savePayment(@Valid @ModelAttribute Payment payment, BindingResult bindingResult) {
    ...
    if ( payment.getId() == null ) {
      if ( paymentService.lastPayment() == null ) {
        ...
        payment.setCode("KMMP" + makeAutoGenerateNumberService.numberAutoGen(null).toString());
      } else {
        //if there is customer in db need to get that customer's code and increase its value
        String previousCode = paymentService.lastPayment().getCode().substring(4);
        payment.setCode("KMMP" + makeAutoGenerateNumberService.numberAutoGen(previousCode).toString());
      }
    }
